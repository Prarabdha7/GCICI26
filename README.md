...
- **Three-tier video: Veo → real stock footage → ColorClip.** `assemble_video()`
  ...
  `PEXELS_API_KEY`, no results), a brand-tinted `ColorClip` is the final
  safety net, the brief's own sanctioned zero-cost alternative. All three
...
| Video | Veo first, real Pexels stock footage second, `moviepy` (pinned `<2.0`) + `ffmpeg` ColorClip third |
...
| Video generation (Veo → Pexels stock footage → moviepy ColorClip) | `app/media/assembly.py`, `app/llm/client.py`, `app/media/stock_video.py` |
